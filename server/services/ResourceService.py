...
class ResourceService(object):
	def __init__(self, app, pageRepository, designService, generalSettingsService, api_only = False):
		self.app = app
		self.api = Api(app)

		self.__init_api(pageRepository, generalSettingsService)

		if not api_only:
			self.__init_webpage(designService)

	def __init_api(self, pageRepository, generalSettingsService):
